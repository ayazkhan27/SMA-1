...
\title{\bfseries A Golden-Ontology Structure-Mapping Memory Turns a Generalist
...
ingests any open ontology---we mount eleven across six domains
($\approx$594{,}000 concepts)---with no per-domain retrieval code. On a
memory-swap benchmark where only the retriever varies, SMA beats an enterprise
RAG and knowledge-graph gauntlet on the rare, long-tail slice across five
unrelated domains (medicine $+33$, finance $+17$, genomics $+16$, cyber $+7$,
legal $+6$ percentage points top-5; all Holm-significant) while matching a
...
structure-mapping memory grounded in a golden domain ontology turns a generalist
...
+ registry + router), demonstrated on eleven ontologies across six domains with
no per-domain retrieval code. Second, a \textbf{memory-swap benchmark} that holds
the language model and prompt fixed and swaps only the retriever, evaluated
against the full enterprise RAG/KG gauntlet. Third, an \textbf{honest
...
(MAC bound $\to$ FAC alignment) with verifiable agency: a structural citation and
an abstain/novelty signal from expectation violation. (d) One universal loader
mounts eleven ontologies; a domain router routes across six domains without
merging them.}
\end{figure}
...
serves all six domains without merging them. The benchmark holds the language
model and prompt fixed and swaps only the retrieval memory among SMA and the
enterprise baselines, so any difference is attributable to retrieval alone. The
gauntlet is the production stack: BM25~\cite{robertson2009bm25}, BGE neural dense
...
\textit{SMA matches a bespoke ontology oracle but beats general-purpose RAG/KG.}
...
parity on top-5 (HPO $\Delta{=}0.000$, $p{=}1.0$; GO $\Delta{=}{+}0.036$,
$p{=}0.31$, Holm). We therefore frame the oracle as a ceiling reference, not an
...
attains tail top-5 of $0.949$ versus $0.606$ for the best enterprise
...
One mechanism serves medicine, genomics, finance, legal and cyber with no
...
ontologies mounted as retrieval geometry; a language model can draft adapter
rules but cannot hallucinate decades of community curation. Relational richness,
...
richly differentiated, as in the curated golden ontologies.
...
threshold-free grounding AUROC. Per-axis paired bootstraps and Holm correction
follow the pre-registration.
...
(\texttt{adapter-v1}, \texttt{prereg-v1/v2}); pre-registrations are committed
before the corresponding runs. Extended per-domain metrics, the cross-system
