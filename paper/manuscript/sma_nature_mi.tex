...
\title{\bfseries Align, Don't Retrieve: Structure-Mapping Memory Grounds Language Models in Curated Ontologies}
...
\includegraphics[width=\linewidth]{design/fig1_overview_crop.pdf}
...
\textit{A mechanism ablation isolates what structure-mapping adds.} To attribute
...
$p{=}2{\times}10^{-4}$), then information-content weighting adds a significant
second increment ($\to 0.982$, $\Delta{=}{+}0.027$, $p{=}0.003$). On
\textbf{genomics} (GO, which does carry typed relations) the same ordering holds
on a smaller scale: lexical $0.750 \to {+}\text{is-a}$ $0.830$
...
relations adding nothing detectable at top-5---consistent with the Phenomizer IC
parity and with the SSB control, where structural alignment is load-bearing
precisely because no lexical or information-content signal is available. We note
the ablation runs at a reduced index scale ($n_\text{index}{=}400$; full scale is
compute-bound), so the robust quantity is the \emph{relative} contribution of
each rung, not the absolute accuracies (Extended Data Table~9).
...
arms ($0.017$--$0.530$ versus best-RAG $0.059$--$0.795$): a structural match
...
\begin{figure}[t]\centering
\includegraphics[width=\linewidth]{design/figM_architecture_crop.pdf}
