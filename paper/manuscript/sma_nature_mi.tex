...
The boundary we draw points to the natural next domain. SMA's null on
flat-tabular fraud is a property of single-record encoding, not of the domain:
real fraud structure is cross-transaction (rings of cards and devices across many
records). Encoding each transaction's graph neighbourhood---predecessor and
successor flows on a public ledger such as Elliptic~\cite{weber2019elliptic}---as
higher-order relations, with a licit/illicit typology as the lattice, supplies
exactly the cross-record structure that flat-tabular retrieval lacks and that
graph neural networks~\cite{kipf2017gcn} exploit; whether SMA's structural
retrieval converts that into a win is the test that closes this boundary.
