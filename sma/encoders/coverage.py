...
def _line_fired(line_lower: str) -> bool:
    if event_classes(line_lower):
        return True
    return any(any(k in line_lower for k in keywords) for keywords in _KEYWORD_ATTRIBUTES)


def rule_coverage(text: str) -> dict:
    """Fraction of non-empty lines that fired at least one frozen class rule.
    ...
    covered = sum(1 for line in lines if _line_fired(line.lower()))
    total = len(lines)
