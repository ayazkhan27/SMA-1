...
- **T5 agent‑drift protocol:** **REVISED 2026‑06‑12 (post‑confirmatory) to use REAL data — see `docs/superpowers/specs/2026-06-12-phase4a-drift-design.md`.** The original synthetic ops‑world design (below) is superseded: a real benchmark proves real drift and avoids the "we built the world" critique. New design: **LongMemEval** (+ LoCoMo) agent‑memory benchmark; four memory variants sharing a DeepSeek backbone (context‑only / RAG‑notes / **Zep‑Graphiti SOTA, run by us on DeepSeek** / SMA); LLM extraction **held constant** (matches every agent‑memory system on the leaderboard) so the experiment isolates the memory *mechanism*; metrics = LongMemEval per‑category accuracy (knowledge‑updates = concept drift, temporal) + update‑recovery/staleness + **SAGE expectation‑violation scored as a standard concept‑drift detector** (prequential test‑then‑learn, ADWIN‑style detection delay/recovery). H4 (SMA resists drift better than generative memories) and H6 (SAGE detects concept drift structurally) tested here. — *Original superseded design:* a seeded synthetic ops world (services, dependencies, injected faults) with known ground‑truth state; three variants; horizons t=10..200; per‑step state‑fidelity F1, contradiction rate, task success vs t.
